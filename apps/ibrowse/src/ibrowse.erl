...
-export([init/1, handle_call/3, handle_cast/2, handle_info/2, terminate/2,
         code_change/3]).
%% API interface
-export([rescan_config/0, rescan_config/1, add_config/1, get_config_value/1,
         get_config_value/2, spawn_worker_process/1, spawn_worker_process/2,
         spawn_link_worker_process/1, spawn_link_worker_process/2, stop_worker_process/1,
         send_req/3, send_req/4, send_req/5, send_req/6, send_req_direct/4, send_req_direct/5,
         send_req_direct/6, send_req_direct/7, stream_next/1, stream_close/1, set_max_sessions/3,
         set_max_pipeline_size/3, set_max_attempts/3, set_dest/3, trace_on/0, trace_off/0,
         trace_on/2, trace_off/2, all_trace_off/0, show_dest_status/0, show_dest_status/1,
         show_dest_status/2, get_metrics/0, get_metrics/2]).
...
-import(ibrowse_lib, [parse_url/1, get_value/3, do_trace/2, log_msg/2]).
...
send_req(Url, Headers, Method, Body, Options, Timeout) ->
    case catch parse_url(Url) of
        #url{host = Host,
            ...
            {SSLOptions, IsSSL} =
                case Protocol == https orelse get_value(is_ssl, Options_1, false) of
                    false ->
                        {[], false};
                    true ->
                        {get_value(ssl_options, Options_1, []), true}
                end,
            try_routing_request(Lb_pid,
                                Parsed_url,
                                Max_sessions,
                                Max_pipeline_size,
                                {SSLOptions, IsSSL},
...
try_routing_request(Lb_pid,
    ...
    case ibrowse_lb:spawn_connection(Lb_pid,
                                     Parsed_url,
                                     Max_sessions,
                                     Max_pipeline_size,
                                     {SSLOptions, IsSSL},
        ...
        {ok, {_Pid_cur_spec_size, _, Conn_Pid}} ->
            RetryRouting =
                fun() ->
                   ...
                   Time_taken_so_far =
                       trunc(round(timer:now_diff(Time_now, Req_start_time) / 1000)),
...
merge_options(Host, Port, Options) ->
    Config_options =
        get_config_value({options, Host, Port}, []) ++ get_config_value({options, global}, []),
...
get_max_sessions(Host, Port, Options) ->
    get_value(max_sessions,
              Options,
              get_config_value({max_sessions, Host, Port}, default_max_sessions())).

get_max_pipeline_size(Host, Port, Options) ->
    get_value(max_pipeline_size,
              Options,
              get_config_value({max_pipeline_size, Host, Port}, default_max_pipeline_size())).

get_max_attempts(Host, Port, Options) ->
    get_value(max_attempts,
              Options,
              get_config_value({max_attempts, Host, Port}, default_max_attempts())).
...
do_send_req(Conn_Pid, Parsed_url, Headers, Method, Body, Options, Timeout) ->
    case catch ibrowse_http_client:send_req(Conn_Pid,
                                            Parsed_url,
                                            Headers,
                                            Method,
                                            ensure_bin(Body),
    ...
    of
        {'EXIT', {timeout, _}} ->
            P_info =
                case catch erlang:process_info(Conn_Pid, [messages, message_queue_len, backtrace])
                of
...
ensure_bin(B) when is_binary(B) ->
    B;
ensure_bin(Fun) when is_function(Fun) ->
    Fun;
ensure_bin({Fun}) when is_function(Fun) ->
    Fun;
ensure_bin({Fun, _} = Body) when is_function(Fun) ->
    Body.
...
send_req_direct(Conn_pid, Url, Headers, Method, Body, Options, Timeout) ->
    case catch parse_url(Url) of
        #url{host = Host, port = Port} = Parsed_url ->
            Options_1 = merge_options(Host, Port, Options),
            case do_send_req(Conn_pid, Parsed_url, Headers, Method, Body, Options_1, Timeout) of
                {error, {'EXIT', {noproc, _}}} ->
...
show_dest_status() ->
    ...
    io:format("~80.80.=s~n", [""]),
    Metrics = get_metrics(),
    lists:foreach(fun({Host, Port, {Lb_pid, _, Tid, Size, _}}) ->
...
show_dest_status(Host, Port) ->
    case get_metrics(Host, Port) of
        {Lb_pid, MsgQueueSize, Tid, Size, {{First_p_sz, First_p_sz}, {Last_p_sz, Last_p_sz}}} ->
...
get_metrics() ->
    Dests =
        lists:filter(fun (#lb_pid{host_port = {Host, Port}})
    ...
    lists:foldl(fun(#lb_pid{host_port = {X_host, X_port}}, X_acc) ->
...
get_metrics(Host, Port) ->
    case ets:lookup(ibrowse_lb, {Host, Port}) of
        ...
        [#lb_pid{pid = Lb_pid, ets_tid = Tid}] ->
            MsgQueueSize =
                case catch process_info(Lb_pid, message_queue_len) of
...
insert_config(Terms) ->
    Fun = fun ({dest, Host, Port, MaxSess, MaxPipe, Options})
                  when is_list(Host), is_integer(Port), is_integer(MaxSess), MaxSess > 0,
                  ...
                  lists:foreach(fun({X, Y}) ->
                                   ets:insert(ibrowse_conf, #ibrowse_conf{key = X, value = Y})
                                end,
                                I);
              ({K, V}) ->
                  ets:insert(ibrowse_conf, #ibrowse_conf{key = K, value = V});
              (X) ->
...
handle_info({trace, Bool, Host, Port}, State) ->
    Fun = fun (#lb_pid{host_port = {H, P}, pid = Pid}, _) when H == Host, P == Port ->
                  catch Pid ! {trace, Bool};
              (_, Acc) ->
                  Acc
          end,
    ets:foldl(Fun, undefined, ibrowse_lb),
    ets:insert(ibrowse_conf, #ibrowse_conf{key = {trace, Host, Port}, value = Bool}),
    {noreply, State};
