...
handshake(Socket, Options) when is_port(Socket) ->
    ...
    case gen_tcp:recv(Socket, 2) of
        ...
        {ok, <<?VERSION, ?USERPASS>>} ->
            ...
            Auth_msg =
                list_to_binary([1, iolist_size(User), User, iolist_size(Password), Password]),
...
connect(Host, Port, Via) when is_binary(Host), is_integer(Port), is_port(Via) ->
    {AddressType, Address} =
    ...
    ok =
        gen_tcp:send(Via,
                     <<?VERSION, ?CONNECT, ?RESERVED, AddressType, Address/binary, Port:16>>),
