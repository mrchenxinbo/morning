...
%%--------------------------------------------------------------------
%% External exports
-export([start_link/1, spawn_connection/6, stop/1]).
%% gen_server callbacks
-export([init/1, handle_call/3, handle_cast/2, handle_info/2, terminate/2,
         code_change/3]).

-record(state,
        {parent_pid, ets_tid, host, port, max_sessions, max_pipeline_size, proc_state}).
...
init([Host, Port]) ->
    ...
    State =
        #state{parent_pid = whereis(ibrowse),
               host = Host,
               port = Port,
...
spawn_connection(Lb_pid,
                 ...
                 SSL_options,
                 Process_options)
    when is_pid(Lb_pid), is_record(Url, url), is_integer(Max_pipeline_size),
...
handle_call({spawn_connection, Url, Max_sess, Max_pipe, SSL_options, Process_options},
    ...
    case Tid_size >= Max_sess of
        ...
        false ->
            ...
            {reply,
             {ok, {1, Ts, Pid}},
             State_1#state{max_sessions = Max_sess, max_pipeline_size = Max_pipe}}
    end;
...
handle_info({trace, Bool}, #state{ets_tid = Tid} = State) ->
    ets:foldl(fun ({{_, _, Pid}, _}, Acc) when is_pid(Pid) ->
...
maybe_create_ets(#state{ets_tid = undefined,
    ...
    ets:insert(ibrowse_lb,
               #lb_pid{host_port = {Host, Port},
                       pid = self(),
                       ets_tid = Tid}),
