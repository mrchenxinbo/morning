...
-export([get_trace_status/2, do_trace/2, do_trace/3, log_msg/2, url_encode/1,
         decode_rfc822_date/1, status_code/1, encode_base64/1, decode_base64/1, get_value/2,
         get_value/3, parse_url/1, printable_date/0, printable_date/1]).
...
status_code(X) when is_list(X) ->
    status_code(list_to_integer(X));
status_code(_) ->
    unknown_status_code.
...
parse_url([H | T], get_username, Url, TmpAcc) when H == $/; H == $? ->
    Path =
        case H of
            $/ ->
                [$/ | T];
...
parse_url([$@ | T], get_username, Url, TmpAcc) ->
    parse_url(T, get_host, Url#url{username = lists:reverse(TmpAcc), password = ""}, []);
...
parse_url([$[ | T], get_password, Url, TmpAcc) ->
    ...
    parse_url(T,
              get_ipv6_address,
              Url#url{host_type = ipv6_address, password = lists:reverse(TmpAcc)},
              []);
parse_url([$@ | T], get_password, Url, TmpAcc) ->
    parse_url(T, get_host, Url#url{password = lists:reverse(TmpAcc)}, []);
parse_url([H | T], get_password, Url, TmpAcc) when H == $/; H == $? ->
...
parse_url([$: | T], get_host, #url{} = Url, TmpAcc) ->
    parse_url(T, get_port, Url#url{host = lists:reverse(TmpAcc)}, []);
parse_url([H | T], get_host, #url{protocol = Prot} = Url, TmpAcc) when H == $/; H == $? ->
    Path =
        case H of
            $/ ->
                [$/ | T];
    ...
    Url#url{host = lists:reverse(TmpAcc),
            ...
            path = Path};
parse_url([H | T], get_port, #url{protocol = Prot} = Url, TmpAcc) when H == $/; H == $? ->
    Path =
        case H of
            $/ ->
                [$/ | T];
...
do_trace(_, Fmt, Args) ->
    io:format("~s -- (~s) - " ++ Fmt, [printable_date(), get(ibrowse_trace_token) | Args]).

-else.

do_trace(true, Fmt, Args) ->
    Fmt_1 = "~s -- (~s) - " ++ Fmt,
    Args_1 = [printable_date(), get(ibrowse_trace_token) | Args],
        ...
        [{"http://[FEDC:BA98:7654:3210:FEDC:BA98:7654:3210]:80/index.html",
         ...
         {"http://[2010:836B:4179::836B:4179]",
          #url{abspath = "http://[2010:836B:4179::836B:4179]",
               ...
               path = "/"}}],
    lists:foreach(fun({Url, Expected_result}) -> ?assertMatch(Expected_result, parse_url(Url))
                  end,
                  Urls).
