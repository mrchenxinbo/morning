...
%%--------------------------------------------------------------------
%% External exports
-export([start_link/1, start_link/2, start/1, start/2, stop/1, send_req/7]).
...
-record(state,
        {host, port, connect_timeout, inactivity_timer_ref, use_proxy = false, proxy_auth_basic,
         ssl_options = [], is_ssl = false, socket, proxy_tunnel_setup = false,
         tunnel_setup_queue = [], reqs = queue:new(), cur_req, status = idle, http_status_code,
         reply_buffer = <<>>, rep_buf_size = 0, streamed_size = 0, recvd_headers = [], status_line,
         raw_headers, is_closing, content_length, deleted_crlf = false, transfer_encoding,
         chunk_size, chunk_size_buffer = <<>>, recvd_chunk_size, interim_reply_sent = false,
         lb_ets_tid, cur_pipeline_size = 0, prev_req_id, proc_state}).
-record(request,
        {url,
         method,
         options,
...
-import(ibrowse_lib, [get_value/2, get_value/3, do_trace/2]).
...
handle_call({send_req, _}, _From, #state{proc_state = ?dead_proc_walking} = State) ->
    shutting_down(State),
    {reply, {error, connection_closing}, State};
handle_call({send_req, {Url, Headers, Method, Body, Options, Timeout}}, From, State) ->
...
handle_info({ssl, _Sock, Data}, State) ->
    handle_sock_data(Data, State);
handle_info({stream_next, Req_id},
            #state{socket = Socket, cur_req = #request{req_id = Req_id}} = State) ->
    _ = do_setopts(Socket, [{active, once}], State),
    {noreply, set_inac_timer(State)};
handle_info({stream_next, _Req_id}, State) ->
    _Cur_req_id =
        case State#state.cur_req of
            #request{req_id = Cur} ->
                Cur;
            _ ->
                undefined
...
handle_info({req_timedout, From}, #state{reqs = Reqs} = State) ->
    ...
    case lists:keysearch(From, #request.from, Reqs_list) of
        ...
        {value, #request{stream_to = StreamTo, req_id = ReqId}} ->
            ...
            State_1 = State#state{proc_state = ?dead_proc_walking},
            shutting_down(State_1),
            Reqs_1 = lists:filter(fun(#request{from = X_from}) -> X_from /= From end, Reqs_list),
        ...
        _ ->
            case parse_11_response(Data, State) of
                {error, Reason} ->
                    ...
                    delayed_stop_timer(),
                    {noreply, State_1};
                #state{cur_req = #request{caller_controls_socket = Ccs}, interim_reply_sent = Irs} =
                    State_1 ->
                    _ = case Irs of
                            true ->
                                active_once(State_1);
                    ...
                    case Ccs of
                        true ->
                            cancel_timer(State_2#state.inactivity_timer_ref,
                                         {eat_message, timeout}),
...
accumulate_response(Data,
                    #state{cur_req =
                               #request{save_response_to_file = Srtf, tmp_file_fd = undefined} =
                                   CurReq,
    ...
    case file:open(TmpFilename, [Mode, delayed_write, raw]) of
        {ok, Fd} ->
            accumulate_response(Data,
                                State#state{cur_req =
                                                CurReq#request{tmp_file_fd = Fd,
                                                               tmp_file_name = TmpFilename}});
        {error, Reason} ->
            {error, {file_open_error, Reason}}
    end;
accumulate_response(Data,
                    #state{cur_req = #request{save_response_to_file = Srtf, tmp_file_fd = Fd},
...
handle_sock_closed(#state{reply_buffer = Buf,
                          reqs = Reqs,
                          http_status_code = SC,
                          is_closing = IsClosing,
                          cur_req =
                              #request{tmp_file_name = TmpFilename, tmp_file_fd = Fd} = CurReq,
                          status = get_body,
                          recvd_headers = Headers,
    ...
    case IsClosing of
        true ->
            ...
            Reply =
                case get_value(give_raw_headers, Options, false) of
...
do_connect(Host,
    ...
    case get_value(socks5_host, Options, undefined) of
        ...
        _ ->
            ...
            Conn = ibrowse_socks5:connect(Host, Port, Options, Sock_options, Timeout),
            case Conn of
                {ok, Sock} ->
...
get_sock_options(Host, Options, SSLOptions) ->
    ...
    Other_sock_options =
        filter_sock_options(SSLOptions ++ Caller_socket_options ++ Ipv6Options),
...
do_send(Req, #state{socket = Sock, is_ssl = true}) ->
    ssl:send(Sock, Req);
...
do_send_body({Source}, State, TE) when is_function(Source) ->
    do_send_body_1(generate_body(Source), State, TE, []);
do_send_body({Source, Source_state}, State, TE) when is_function(Source) ->
    do_send_body_1(generate_body({Source, Source_state}), State, TE, []);
...
send_req_1(From,
    ...
    {Host_1, Port_1, State_1} =
        case get_value(proxy_host, Options, false) of
            ...
            PHost ->
                ...
                {PHost,
                 get_value(proxy_port, Options, 80),
                 State#state{use_proxy = true, proxy_auth_basic = AuthBasic}}
        end,
    ...
    case do_connect(Host_1, Port_1, Options, State_2, Conn_timeout) of
        {ok, Sock} ->
            ...
            State_3 = State_2#state{socket = Sock, connect_timeout = Conn_timeout},
...
%% Then send request
send_req_1(From,
           #url{host = Server_host, port = Server_port} = Url,
           Headers,
           Method,
           Body,
           Options,
           Timeout,
           #state{proxy_tunnel_setup = false,
                  use_proxy = true,
                  is_ssl = true} =
               State) ->
    Ref = case Timeout of
    ...
    {Req, Body_1} =
        make_request(connect, Pxy_auth_headers, Path, Path, [], Options, State_1, undefined),
        ...
        ok ->
            case do_send_body(Body_1, State_1, TE) of
                {ok, _Sent_body} ->
                    ...
                    State_2 =
                        State_1_1#state{status = get_header,
                                        cur_req = NewReq,
                                        proxy_tunnel_setup = in_progress,
                                        tunnel_setup_queue =
                                            [{From, Url, Headers, Method, Body, Options, Timeout}]},
...
send_req_1(From,
           ...
           Options,
           Timeout,
           #state{proxy_tunnel_setup = in_progress, tunnel_setup_queue = Q} = State) ->
    ...
    {noreply,
     State#state{tunnel_setup_queue =
                     [{From, Url, Headers, Method, Body, Options, Timeout} | Q]}};
send_req_1(From,
           #url{abspath = AbsPath, path = RelPath} = Url,
           Headers,
           Method,
           Body,
           Options,
           Timeout,
           #state{status = Status, socket = Socket} = State) ->
    ...
    {StreamTo, Caller_controls_socket} =
        case get_value(stream_to, Options, undefined) of
            {Caller, once} when is_pid(Caller) or is_atom(Caller) ->
                Async_pid_rec = {{req_id_pid, ReqId}, self()},
                true = ets:insert(ibrowse_stream, Async_pid_rec),
                {Caller, true};
            undefined ->
                {undefined, false};
            Caller when is_pid(Caller) or is_atom(Caller) ->
                {Caller, false};
            Stream_to_inv ->
    ...
    {Req, Body_1} =
        make_request(Method, Headers_1, AbsPath, RelPath, Body, Options, State, ReqId),
    NewReq =
        #request{url = Url,
                 method = Method,
                 stream_to = StreamTo,
            ...
            case do_send_body(Body_1, State, TE) of
                {ok, Sent_body} ->
                    ...
                    State_3 =
                        case Status of
                            idle ->
                                State_2#state{status = get_header, cur_req = NewReq_1};
                            _ ->
                                State_2
                        end,
                    case StreamTo of
...
maybe_modify_headers(#url{host = Host, port = Port} = Url,
    ...
    case get_value(headers_as_is, Options, false) of
        false ->
            ...
            HostHeaderValue =
                case lists:keysearch(host_header, 1, Options) of
                    false ->
                        case Port of
                            80 ->
                                Host;
...
add_auth_headers(#url{username = User, password = UPw}, Options, Headers, State) ->
    Headers_1 =
        case User of
            undefined ->
...
make_request(Method,
    ...
    Headers_1 =
        case lists:keysearch("content-length", 1, Headers_0) of
            false
                when (Body =:= [] orelse Body =:= <<>>)
                     andalso (Method =:= post orelse Method =:= put) ->
                [{"content-length", "Content-Length", "0"} | Headers_0];
            false when is_binary(Body) orelse is_list(Body) ->
                [{"content-length", "Content-Length", integer_to_list(iolist_size(Body))}
                 | Headers_0];
            _ ->
    ...
    {Headers_2, Body_1} =
        case is_chunked_encoding_specified(Options) of
            ...
            true ->
                Chunk_size_1 =
                    case get_value(transfer_encoding, Options) of
                        chunked ->
                            5120;
                        {chunked, Chunk_size} ->
                            Chunk_size
                    end,
                {[{Y, Z} || {X, Y, Z} <- Headers_1, X /= "content-length"]
                 ++ [{"Transfer-Encoding", "chunked"}],
                 chunk_request_body(Body, Chunk_size_1)}
        end,
    Headers_3 =
        case lists:member({include_ibrowse_req_id, true}, Options) of
            true ->
...
cons_headers([{basic_auth, {U, P}} | T], Acc) ->
    cons_headers(T,
                 [{"Authorization", ["Basic ", ibrowse_lib:encode_base64(U ++ ":" ++ P)]} | Acc]);
...
chunk_request_body(Body, _ChunkSize) when is_tuple(Body) orelse is_function(Body) ->
    Body;
...
parse_response(Data,
    ...
    case scan_header(Acc, Data) of
        {yes, Headers, Data_1} ->
            ...
            do_trace("Recvd headers~n--- Headers Begin ---~n~s~n--- Headers End ---~n~n",
                     [Headers]),
            {HttpVsn, StatCode, Headers_1, Status_line, Raw_headers} = parse_headers(Headers),
            do_trace("HttpVsn: ~p StatusCode: ~p Headers_1 -> ~1000.p~n",
            ...
            IsChunked =
                lists:any(fun(Enc) -> string:strip(Enc) =:= "chunked" end,
            ...
            case get_header_value("content-length", LCHeaders, undefined) of
                _ when Method == connect, hd(StatCode) == $2 ->
                    {_, Reqs_1} = queue:out(Reqs),
                    cancel_timer(T_ref),
                ...
                _ when Method == connect ->
                    {_, Reqs_1} = queue:out(Reqs),
                    do_error_reply(State#state{reqs = Reqs_1}, {error, proxy_tunnel_failed}),
                    {error, proxy_tunnel_failed};
                _ when Method =:= head, Head_response_with_body =:= false ->
                ...
                _ when hd(StatCode) =:= $1 ->
                    ...
                    %% response.
                    send_async_headers(ReqId, StreamTo, Give_raw_headers, State_1),
                    do_trace("Recvd a status code of ~p. Ignoring and waiting for a proper response~n",
                             [StatCode]),
                    parse_response(Data_1, State_1#state{recvd_headers = [], status = get_header});
                _ when StatCode =:= "204"; StatCode =:= "304" ->
                ...
                _ when IsChunked ->
                    ...
                    of
                        {error, Reason} ->
                            fail_pipelined_requests(State_1,
                                                    {error,
                                                     {Reason, {stat_code, StatCode}, Headers_1}}),
                            {error, Reason};
                        State_2 ->
                            State_2
                    end;
                undefined when HttpVsn =:= "HTTP/1.0"; ConnClose =:= "close" ->
                    send_async_headers(ReqId, StreamTo, Give_raw_headers, State_1),
...
send_queued_requests([], State) ->
    ...
    State#state{tunnel_setup_queue = []};
send_queued_requests([{From, Url, Headers, Method, Body, Options, Timeout} | Q], State) ->
...
is_connection_closing("HTTP/0.9", _) ->
    true;
is_connection_closing(_, "close") ->
    true;
is_connection_closing("HTTP/1.0", "false") ->
    true;
is_connection_closing(_, _) ->
    false.
    ...
    case scan_crlf(Chunk_sz_buf, DataRecvd) of
        {yes, ChunkHeader, Data_1} ->
            ...
            parse_11_response(Data_1,
                              State_1#state{chunk_size_buffer = <<>>,
                                            deleted_crlf = true,
                                            recvd_chunk_size = 0,
...
parse_11_response(DataRecvd,
    ...
    case scan_crlf(Buf, DataRecvd) of
        {yes, _, NextChunk} ->
            ...
            State_2 =
                State_1#state{chunk_size = chunk_start,
                              chunk_size_buffer = <<>>,
                              deleted_crlf = true},
...
handle_response(#request{from = From,
                         stream_to = StreamTo,
                         req_id = ReqId,
                         response_format = Resp_format,
                         save_response_to_file = SaveResponseToFile,
    ...
    {Resp_headers_1, Raw_headers_1} =
        maybe_add_custom_headers(Status_line, RespHeaders, Raw_headers, Options),
    Give_raw_req = get_value(return_raw_request, Options, false),
    Reply =
        case get_value(give_raw_headers, Options, false) of
            true when Give_raw_req == false ->
...
handle_response(#request{from = From,
    ...
    Body = RepBuf,
    {Resp_headers_1, Raw_headers_1} =
        maybe_add_custom_headers(Status_line, Resp_headers, Raw_headers, Options),
    Give_raw_req = get_value(return_raw_request, Options, false),
    Reply =
        case get_value(give_raw_headers, Options, false) of
            true when Give_raw_req == false ->
...
set_cur_request(#state{reqs = Reqs, socket = Socket} = State) ->
    case queue:peek(Reqs) of
        ...
        {value, #request{caller_controls_socket = Ccs} = NextReq} ->
            _ = Ccs =:= true andalso do_setopts(Socket, [{active, once}], State),
...
parse_headers_1([$\n, H | T], [$\r | L], Acc) when H =:= 32; H =:= $\t ->
    parse_headers_1(lists:dropwhile(fun(X) -> is_whitespace(X) end, T), [32 | L], Acc);
parse_headers_1([$\n, H | T], L, Acc) when H =:= 32; H =:= $\t ->
    parse_headers_1(lists:dropwhile(fun(X) -> is_whitespace(X) end, T), [32 | L], Acc);
parse_headers_1([$\n | T], [$\r | L], Acc) ->
...
get_crlf_crlf_pos(<<$\n, $\n, _/binary>>, Pos) ->
    {yes_dodgy, Pos};
get_crlf_crlf_pos(<<_, Rest/binary>>, Pos) ->
    get_crlf_crlf_pos(Rest, Pos + 1);
get_crlf_crlf_pos(<<>>, _) ->
    no.
...
get_crlf_pos(<<$\n, _/binary>>, Pos) ->
    {yes, 1, Pos};
get_crlf_pos(<<_, Rest/binary>>, Pos) ->
    get_crlf_pos(Rest, Pos + 1);
get_crlf_pos(<<>>, _) ->
    no.

fmt_val(L) when is_list(L) ->
    L;
fmt_val(I) when is_integer(I) ->
    integer_to_list(I);
fmt_val(A) when is_atom(A) ->
    atom_to_list(A);
fmt_val(B) when is_binary(B) ->
    B;
fmt_val(Term) ->
...
maybe_add_custom_headers(Status_line, Headers, Raw_headers, Opts) ->
    ...
    Raw_headers_1 =
        case Custom_headers of
            [_ | _] when is_binary(Raw_headers) ->
                Custom_headers_bin =
                    list_to_binary(string:join([[X, $:, Y] || {X, Y} <- Custom_headers], "\r\n")),
...
do_error_reply(#state{reqs = Reqs, tunnel_setup_queue = Tun_q} = State, Err) ->
    ...
    lists:foreach(fun(#request{from = From,
                               stream_to = StreamTo,
                               req_id = ReqId,
                               response_format = Resp_format}) ->
                  ...
                  ReqList),
    lists:foreach(fun({From, _Url, _Headers, _Method, _Body, _Options, _Timeout}) ->
...
dec_pipeline_counter(#state{cur_pipeline_size = Pipe_sz,
                         ...
                         State)
    when Tid /= undefined, Proc_state /= ?dead_proc_walking ->
    Ts = os:timestamp(),
    catch ets:insert(Tid, {{Pipe_sz - 1, os:timestamp(), self()}, []}),
    catch ets:select_delete(Tid,
                            [{{{'_', '$2', '$1'}, '_'},
                              [{'==', '$1', {const, self()}}, {'<', '$2', {const, Ts}}],
                              [true]}]),
    State#state{cur_pipeline_size = Pipe_sz - 1};
dec_pipeline_counter(State) ->
    State.
...
to_binary({X, _}) when is_function(X) ->
    to_binary(X);
to_binary(X) when is_function(X) ->
    <<"body generated by function">>;
