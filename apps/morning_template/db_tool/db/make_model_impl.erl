...
inner_create_model(#proto_define{name=Name, attrs=ModelAttrList, keys=KeyList, type=Type}, _DBMod)->
	{FILE_NAME, RECORD_NAME} = get_proto_name(Name),
	case file:open(?MODEL_SRC_ROOT++FILE_NAME++".erl", [write]) of
		{ok, File} ->
			...
			file:close(File);
		{error, Reason} ->
			io:format("~n!!!!!!!!!!!!!!!!!!!!!!!!!!!!!-  ERROR  -!!!!!!!!!!!!!!!!!!!!!!!!!!!!!~nOpen ~s.erl Error,~n Reason:~p~n~n", [FILE_NAME, Reason])
	end,
	io:format("Complete auto-coding ~s.erl.~n", [FILE_NAME]);
...
inner_create_model(#model_define{name=Name, attrs=ModelAttrList, primary_keys=PrimaryKeys}, _DBMod)->
	...
	case file:open(?MODEL_SRC_ROOT++?MODELPREFIX++RECORD_NAME++".erl", [write]) of
		{ok, File} ->
			...
			SQL_INSERT_AUTO_KEY = pack_insert1(TABLEARGSSTRING, TypeArgList),
			...
			KVList =    [
							...
							{"?SQL_INSERT_AUTO_KEY",    SQL_INSERT_AUTO_KEY},
...
pack_insert1(TABLEARGSSTRING, [_|TypeArgList])->
	[_|NEWTABLEARGSSTRING]= string:split(TABLEARGSSTRING, ","), 
	"("++NEWTABLEARGSSTRING++") VALUES " ++ pack_values_of_insert0(TypeArgList) ++ ";".
...
get_proto_name(Name) when is_atom(Name) ->
	get_proto_name(atom_to_list(Name));
get_proto_name(Name) ->
	case string:substr(Name, 1, 6) of
		?PROTOPREFIX -> {Name, Name};
		_ -> {?PROTOPREFIX++Name, Name}
	end.
...
record_to_float(RecordList) ->
	case lists:keyfind(?RECORD_NAME,1,?TO_FLOAT_RECORD_LIST) of
		false ->
			RecordList;
		{_,FloatFields} ->
			FieldList = record_info(fields,?RECORD_NAME),
...
insert_auto(?THIS)->
		mysql_client:insert(?RECORD_NAME, \"?SQL_INSERT_AUTO_KEY\").
...
read(?PRIMARYKEY_ARGS)->
	Columns = string:join([\"`\" ++ atom_to_list(Key) ++ \"`\" || Key <- record_info(fields, ?RECORD_NAME)], \",\"),
	case mysql_client:select_columns(?RECORD_NAME, Columns, \"?SQL_SELECT0\") of
		{ok,RowList}->
            unpack_rows(RowList);
		_ ->
			[]
...
unpack_rows(RowList, FieldList) ->
	Fileds = record_info(fields, ?RECORD_NAME),
	...
	case FieldList of
		...
		_ ->
			Record = lists:foldl(fun({Filed, Value}, AccTmp) ->
						 ...
						 end, #?RECORD_NAME{}, lists:zip(FieldList, Row))
	end,
